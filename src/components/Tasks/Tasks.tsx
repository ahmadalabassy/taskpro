...
const Tasks = () => {
  ...
  const generateMockTasks = (count: number): TaskData[] => {
    return Array.from({ length: count }, () => ({
      ...
      status: faker.helpers.arrayElement(["Planned", "In Progress", "Completed", "On Pause", "Under Review"]),
      ...
      completedDate: faker.helpers.arrayElement(["", faker.date.future().toISOString().split("T")[0]]),
    }));
  };
 ...
 style={{ backgroundColor: statusColors[status] || "#ccc" }}                  ></span>
                  <h2 className={`fw-bold fs-5 mb-0 ${styles.textColor}`}>{status}</h2>
                </div>
