...
export default function TasksProgress({ task }: TaskCardProps) {
    ...
    const [selectedMember, setSelectedMember] = useState<TeamMemberCardProps | null>(null);
    ...
    return <Card className={styles.taskCard}>
        <Card.Body className={styles.cardBody}>
            <div className={styles.cardHeader}>
                <h3 className={styles.title}>{title}</h3>
                <span className={`${styles.badge} ${statusBadgeClass}`} data-testid={`status-badge-${status}`}>
                    ...
                    <div className={styles.avatars}>
                        {displayMembers.map((member, index) => (
                            <OverlayTrigger
                                key={member.id}
                                placement="top"
                                overlay={<Tooltip id={`tooltip-${member.id}`}>{member.name}</Tooltip>}
                                delay={{ show: 250, hide: 400 }}
                            >
                                <div className={styles.avatarWrapper} style={{ zIndex: displayMembers.length - index }}>
                                    <img
